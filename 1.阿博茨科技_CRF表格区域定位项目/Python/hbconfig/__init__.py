    ...
    class __HBConfigMeta:
        ...
        def read_file(self, fname):
            ...
            if len(config_files) == 0 and self.is_new is False:
                raise IOError("No such files start filename '%s'" % fname)
            else:
        ...
        def __repr__(self):
            if self.config is None:
                raise IOError("No such files start filename '%s'" % self.base_fname)
            else:
                return "Read config file name: %s\n%s" % (self.read_fname, json.dumps(self.config, indent=4))
...
class Config(object):
    __metaclass__ = HBConfigMeta
    pass


class SubConfig:

    def __init__(self, d, get_tag=None):
