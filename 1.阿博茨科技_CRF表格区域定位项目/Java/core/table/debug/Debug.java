package com.abcft.pdfextract.core.table.debug;

import com.abcft.pdfextract.core.table.*;
import com.abcft.pdfextract.core.model.Rectangle;
import com.abcft.pdfextract.core.table.detectors.NurminenDetectionAlgorithm;
import com.abcft.pdfextract.core.table.extractors.BasicExtractionAlgorithm;
...
public class Debug {
    ...
    private static void debugIntersections(Graphics2D g, Page page) {
        int i = 0;
    ...
    private static void debugTextChunks(Graphics2D g, Page page) {
        List<TabularTextChunk> chunks = TabularUtils.mergeWords(page.getText(), page.getVerticalRulings());
        drawShapes(g, chunks);
    }

    @Deprecated
    private static void debugSpreadsheets(Graphics2D g, TabularPage page) {
    ...
    private static void debugCells(Graphics2D g, Rectangle area, Page page) {
        ...
        List<Cell> cells = Ruling.findCells(h, v);
        drawShapes(g, cells);
    }

    private static void debugDetectedTables(Graphics2D g, Page page, String outPath) {
        NurminenDetectionAlgorithm detectionAlgorithm = new NurminenDetectionAlgorithm();
        File outDir = new File(outPath).getParentFile();
        List<Rectangle> tables = detectionAlgorithm.detect(page,
                new File(outDir, removeExtension(outPath)).getAbsolutePath());
        drawShapes(g, tables);
    }
    ...
    private static void debugProjectionProfile(Graphics2D g, Page page) {
        ...
        ProjectionProfile profile = new ProjectionProfile(page,
                TabularUtils.mergeWords(page.getText(), page.getVerticalRulings()), horizSmoothKernel * 1.5f,
                vertSmoothKernel);
    ...
    public static void renderPage(String pdfPath, String outPath, int pageNumber, Rectangle area,
        ...
        TabularTableExtractor oe = new TabularTableExtractor(document);

        TabularPage page = oe.extract(pageNumber + 1);
        ...
        BufferedImage image = DebugHelper.pageConvertToImage(p, 72, ImageType.RGB);

        Graphics2D g = (Graphics2D) image.getGraphics();
       /*/////////////////////////////////////////////////////////
        LinkedList<Rectangle> clipAreas = GridlessSheetExtractionAlgorithm.ClipAreaFeature.getClipAreas(page);
        drawShapes(g, clipAreas);
        */////////////////////////////////////////////////////////
        ...
        if (drawDetectedTables) {
            debugDetectedTables(g, page, outPath);
        }
    ...
    public static void main(String[] args) throws IOException {
        ...
        try {
            ...
            if (line.hasOption('p')) {
                pages = TableUtils.parsePagesOption(line.getOptionValue('p'));
            } else {
     //           pages.add(1);
            }
            ...
            Rectangle area = null;
            if (line.hasOption('a')) {
                List<Float> f = NumberUtil.parseFloatList(line.getOptionValue('a'));
                if (f.size() != 4) {
                    throw new ParseException("area parameters must be top,left,bottom,right");
                }
                area = new Rectangle(f.get(1), f.get(0), f.get(3) - f.get(1), f.get(2) - f.get(0));
            }
