...
import com.abcft.pdfextract.core.table.debug.Debug;
...
public class NurminenDetectionAlgorithm implements DetectionAlgorithm {
    ...
    private static final int IMAGE_DPI = 72;
    ...
    private static final class TextEdge extends Line2D.Float {
        ...
        public String toString() {
            ...
            String rv = formatter.format("%s[x1=%f y1=%f x2=%f y2=%f intersectingTextRowCount=%d]",
                    this.getClass().getSimpleName(), this.x1, this.y1, this.x2, this.y2, this.intersectingTextRowCount).toString();
            formatter.close();
            return rv;
    ...
    public List<Rectangle> detect(Page page, String debugPrefix) {
        boolean debug = debugPrefix != null;
        ...
        BufferedImage grayImage, noTextImage;
        PDPage pdfPage = page.getPDPage();
        try {
            grayImage = DebugHelper.pageConvertToImage(pdfPage, IMAGE_DPI, ImageType.GRAY);
        } catch (IOException e) {
            return new ArrayList<Rectangle>();
        }

        List<Ruling> horizontalRulings = this.getHorizontalRulings(grayImage);
        if (debug) {
            try {
                Graphics2D g = (Graphics2D) grayImage.getGraphics();
                Debug.drawShapes(g, horizontalRulings);
                ImageIO.write(grayImage, "PNG", new File(debugPrefix + "-hr.png"));
                g.dispose();
            } catch (IOException e) {
                // Ignore debug error
            }
        ...
        try {
            noTextImage = removeText(pdfPage);
        } catch (Exception e) {
            return new ArrayList<Rectangle>();
        }

        List<Ruling> verticalRulings = this.getVerticalRulings(noTextImage);

        if (debug) {
            try {
                Graphics2D g2 = (Graphics2D) noTextImage.getGraphics();
                Debug.drawShapes(g2, verticalRulings);
                ImageIO.write(grayImage, "PNG", new File(debugPrefix + "-vr.png"));
                g2.dispose();

                Graphics2D g1 = (Graphics2D) grayImage.getGraphics();
                Debug.drawShapes(g1, verticalRulings);
                ImageIO.write(grayImage, "PNG", new File(debugPrefix + "-rule.png"));
                g1.dispose();
            } catch (IOException e) {
                // Ignore debug error
            }
        ...
        if (allEdges.size() > 0) {
            ...
            TableUtils.snapPoints(allEdges, POINT_SNAP_DISTANCE_THRESHOLD, POINT_SNAP_DISTANCE_THRESHOLD);
            ...
            List<? extends Rectangle> cells = Ruling.findCells(horizontalRulings, verticalRulings);
        ...
        List<TabularTextChunk> textChunks = TabularUtils.mergeWords(page.getText());
        ...
        do {
            ...
            for (Iterator<TabularLine> iterator = lines.iterator(); iterator.hasNext();) {
                TabularLine textRow = iterator.next();
    ...
    private Rectangle getTableFromText(List<TabularLine> lines,
        ...
        TabularLine prevRow = null;
        TabularLine firstTableRow = null;
        TabularLine lastTableRow = null;
    ...
    private RelevantEdges getRelevantEdges(TextEdges textEdges, List<TabularLine> lines) {
        List<TextEdge> leftTextEdges = textEdges.get(TextEdge.LEFT);
        List<TextEdge> midTextEdges = textEdges.get(TextEdge.MID);
        List<TextEdge> rightTextEdges = textEdges.get(TextEdge.RIGHT);
    ...
    private TextEdges getTextEdges(List<TabularLine> lines) {
        ...
        Map<Integer, List<TabularTextChunk>> currLeftEdges = new HashMap<Integer, List<TabularTextChunk>>();
        Map<Integer, List<TabularTextChunk>> currMidEdges = new HashMap<Integer, List<TabularTextChunk>>();
        Map<Integer, List<TabularTextChunk>> currRightEdges = new HashMap<Integer, List<TabularTextChunk>>();

        for (TabularLine textRow : lines) {
            for (TabularTextChunk text : textRow.getTextElements()) {
                ...
                List<TabularTextChunk> leftEdge = currLeftEdges.get(left);
                if (leftEdge == null) {
                    leftEdge = new ArrayList<TabularTextChunk>();
                    currLeftEdges.put(left, leftEdge);
                }
                leftEdge.add(text);

                List<TabularTextChunk> midEdge = currMidEdges.get(mid);
                if (midEdge == null) {
                    midEdge = new ArrayList<TabularTextChunk>();
                    currMidEdges.put(mid, midEdge);
                }
                midEdge.add(text);

                List<TabularTextChunk> rightEdge = currRightEdges.get(right);
                if (rightEdge == null) {
                    rightEdge = new ArrayList<TabularTextChunk>();
                    currRightEdges.put(right, rightEdge);
                }
                rightEdge.add(text);

                // now see if this text chunk blows up any other edges
                for (Iterator<Map.Entry<Integer, List<TabularTextChunk>>> iterator = currLeftEdges.entrySet().iterator(); iterator.hasNext();) {
                    Map.Entry<Integer, List<TabularTextChunk>> entry = iterator.next();
                    Integer key = entry.getKey();
                    if (key > left && key < right) {
                        iterator.remove();
                        List<TabularTextChunk> edgeChunks = entry.getValue();
                        if (edgeChunks.size() >= REQUIRED_TEXT_LINES_FOR_EDGE) {
                            TabularTextChunk first = edgeChunks.get(0);
                            TabularTextChunk last = edgeChunks.get(edgeChunks.size() - 1);
                ...
                for (Iterator<Map.Entry<Integer, List<TabularTextChunk>>> iterator = currMidEdges.entrySet().iterator(); iterator.hasNext();) {
                    Map.Entry<Integer, List<TabularTextChunk>> entry = iterator.next();
                    Integer key = entry.getKey();
                ...
                for (Iterator<Map.Entry<Integer, List<TabularTextChunk>>> iterator = currRightEdges.entrySet().iterator(); iterator.hasNext();) {
                    Map.Entry<Integer, List<TabularTextChunk>> entry = iterator.next();
                    Integer key = entry.getKey();
                    if (key > left && key < right) {
                        iterator.remove();
        ...
        // add the leftovers
        addSideOvers(currLeftEdges, leftTextEdges, lines);

        addSideOvers(currLeftEdges, midTextEdges, lines);

        addSideOvers(currRightEdges, rightTextEdges, lines);

        return new TextEdges(leftTextEdges, midTextEdges, rightTextEdges);
    }

    private static void addSideOvers(Map<Integer, List<TabularTextChunk>> edgesMap, List<TextEdge> edges, List<TabularLine> lines) {
        for (Map.Entry<Integer, List<TabularTextChunk>> entry : edgesMap.entrySet()) {
            Integer key = entry.getKey();
            List<TabularTextChunk> edgeChunks = entry.getValue();
            if (edgeChunks.size() >= REQUIRED_TEXT_LINES_FOR_EDGE) {
                TabularTextChunk first = edgeChunks.get(0);
    ...
    private List<Rectangle> getTableAreasFromCells(List<? extends Rectangle> cells) {
        ...
        for (List<Rectangle> cellGroup : cellGroups) {
            ...
            tableAreas.add(new Rectangle(left, top, right - left, bottom - top));
        }
    ...
    private static BufferedImage removeText(PDPage page) throws IOException {
        ...
        for (Object token : tokens)
        {
            if( token instanceof Operator)
            {
                ...
                if (op.getName().equals("ET")) {
                    inText = false;
                    continue;
                } else if (op.getName().equals("BT")) {
                    inText = true;
                }
                */
                // The following algorithm seemed not working...
        ...
        COSDictionary pageObj = page.getCOSObject();
        COSBase contents = pageObj.getDictionaryObject(COSName.CONTENTS);
        ...
        BufferedImage noTextImage;
        try {
            noTextImage = DebugHelper.pageConvertToImage(page, IMAGE_DPI, ImageType.GRAY);
            document.close();
        } catch (Exception e) {
            noTextImage = null;
        }
        ...
        pageObj.setItem(COSName.CONTENTS, contents);
        return noTextImage;
    }
}